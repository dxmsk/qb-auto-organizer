...
本版本与 `dxmsk/qb-auto-organizer` 保持可选联动：自动整理插件提交整理前会读取本插件的只读任务状态；首次秒传等待或处理中时，整理会短暂让路，首次失败后允许整理入队。之后秒传重试成功会通知自动整理插件移除对应队列记录，并由 `TransferIntercept` 拦截已经开始的重复整理。秒传失败、源文件不存在、插件未安装或联动开关关闭时，自动整理仍按原逻辑工作。两插件不互相导入代码，任一插件均可单独启用。
